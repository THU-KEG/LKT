...
class TrainDataLoader(object):
    ...
    def __init__(self):
        ...
        with open(config_file) as i_f:
            i_f.readline()
            _, _, knowledge_n,_ = i_f.readline().split(',')
    ...
    def next_batch(self):
        ...
        for count in range(self.batch_size):   
            ...
            ys.append(y)
            kn_id = [i - 1 for i in log['knowledge_code']]
            kn_ids.append(kn_id)
            
        self.ptr += self.batch_size
        return torch.LongTensor(input_stu_ids), torch.LongTensor(input_exer_ids), torch.Tensor(input_knowedge_embs), torch.LongTensor(ys),torch.LongTensor(),torch.LongTensor(kn_ids) 
...
class ValTestDataLoader(object):
    ...
    def next_batch(self):
        ...
        for log in logs:
            ...
            ys.append(y)
        self.ptr += 1
        return torch.LongTensor(input_stu_ids), torch.LongTensor(input_exer_ids), torch.Tensor(input_knowledge_embs), torch.FloatTensor(ys), torch.LongTensor(), torch.LongTensor(kn_ids)
